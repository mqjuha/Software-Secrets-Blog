...
export default function PostPage(){
    ...
    useEffect(() => {
        fetch(`http://localhost:10000/blog/${id}`)
          .then(response => {
            response.json().then(info => {
              ...
              if (info.nav.nav_type === 'TimelineHor') {
                ...
                fetch(`http://localhost:10000/timelineHor/${info.nav.element}`)
                ...
                fetch(`http://localhost:10000/timelineVer/${info.nav.element}`)
                .then(response => response.json())
                .then(timelineVer => {
                ...
                fetch(`http://localhost:10000/tableComponent/${info.nav.element}`)
                .then(response => response.json())
                .then(tableComp => {
              ...
              if (info.references.length !== 0) {
                const refPromises = info.references.map(ref_id => {
                  return fetch(`http://localhost:10000/reference/${ref_id}`)
                    .then(response => response.json());
                });
